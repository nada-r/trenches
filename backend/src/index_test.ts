...
server.listen(EnvVars.Port, async () => {
  ...
  const testAddCaller = await callerService.getOrCreateCaller(
    `${Math.floor(Math.random() * 1000000)}`,
   `RandomUser${Math.floor(Math.random() * 10000)}`,
   "https://trenches.fra1.cdn.digitaloceanspaces.com/Shrek.jpg");
     console.log("🚀 ~ server.listen ~ testAddCaller:", testAddCaller)
